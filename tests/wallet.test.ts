...
describe("generateWalletBundle", () => {
  ...
  it("supports requesting a single role", async () => {
    const bundle = await generateWalletBundle({ appKey, solana: ["funds"] });
    expect(Object.keys(bundle.solana!)).toEqual(["funds"]);
    expect(bundle.evm).toBeUndefined();
  });

  it("flattens to public keys + ciphertext only (no plaintext secrets)", async () => {
    const bundle = await generateWalletBundle({ appKey, solana: ["funds"], evm: ["funds"] });
...
describe("decrypt-to-sign", () => {
  it("reconstructs a usable Solana keypair", async () => {
    const bundle = await generateWalletBundle({ appKey, solana: ["funds"] });
    const wallet = bundle.solana!.funds!;
    const kp = await toSolanaKeypair(wallet, appKey);
    expect(kp).toBeInstanceOf(Keypair);
    expect(kp.publicKey.toBase58()).toBe(wallet.publicKey);
  });

  it("decrypts the EVM private key as 0x hex", async () => {
    const bundle = await generateWalletBundle({ appKey, evm: ["signing"] });
    const secret = await decryptWalletSecret(bundle.evm!.signing!, appKey);
